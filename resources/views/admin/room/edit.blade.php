        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <form method="post" action="{{route('admin.room.update',$rooms->id)}}">
                        @csrf
                        <table class="table table-bordered" >
                            <tr>
                                <th>Title<span class="text-danger">*</span></th>
                                <td><input value="{{$rooms->title}}" name="title" type="text" placeholder="title Name" class="form-control" />
                                    @error('title')
                                        ...
                                        <optgroup label="Choose Location">
                                            @if($locations && $locations -> count() > 0 )
                                                @foreach($locations as $location)
                                                    <option value="{{$location->id}}" @if($location->id == $rooms->location_id) selected @endif > {{$location -> address}} </option>
                                                @endforeach
                                            @endif
                                        </optgroup>
                                    </select>
                                        ...
                                        <optgroup label="Choose Room Type">
                                            @if($roomTypes && $roomTypes -> count() > 0 )
                                                @foreach($roomTypes as $roomType)
                                                    <option value="{{$roomType -> id}}" @if($roomType->id == $rooms->room_type_id) selected @endif> {{$roomType -> title}} </option>
                                                @endforeach
                                            @endif
                                        </optgroup>
